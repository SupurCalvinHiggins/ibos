...
usize uart_receive_count = 0;
usize uart_transmit_count = 0;

//DEMO_packet_t testcache[16];
//usize testcachesize = 0;
...
void DEMO_uart_transmit(DEMO_packet_t packet) {
    ...
    assert(packet.type == 0 || packet.type == 1);
...
void DEMO_uart_handler(void) {
    ...
    // Transmit
    if ((UART4->ISR & USART_ISR_TXE) && transmit_size != sizeof(DEMO_packet_t)) {
        assert(transmit_size <= sizeof(DEMO_packet_t));
...
void DEMO_uart_initialize(void) {
    ...
    transmit_size = sizeof(DEMO_packet_t);
